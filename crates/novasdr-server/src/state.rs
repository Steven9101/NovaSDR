...
pub struct AudioParamsAtomic {
    l: std::sync::atomic::AtomicI32,
    r: std::sync::atomic::AtomicI32,
    m_bits: std::sync::atomic::AtomicU64,
    mute: std::sync::atomic::AtomicU8,
    squelch_enabled: std::sync::atomic::AtomicU8,
    demodulation: std::sync::atomic::AtomicU8,
    agc_speed: std::sync::atomic::AtomicU8,
    agc_attack_ms_bits: std::sync::atomic::AtomicU32,
    agc_release_ms_bits: std::sync::atomic::AtomicU32,
}

impl AudioParamsAtomic {
    const NONE_F32_BITS: u32 = u32::MAX;

    pub fn new(initial: AudioParams) -> Self {
        Self {
            l: std::sync::atomic::AtomicI32::new(initial.l),
            r: std::sync::atomic::AtomicI32::new(initial.r),
            m_bits: std::sync::atomic::AtomicU64::new(initial.m.to_bits()),
            mute: std::sync::atomic::AtomicU8::new(initial.mute as u8),
            squelch_enabled: std::sync::atomic::AtomicU8::new(initial.squelch_enabled as u8),
            demodulation: std::sync::atomic::AtomicU8::new(Self::demod_to_u8(initial.demodulation)),
            agc_speed: std::sync::atomic::AtomicU8::new(Self::agc_to_u8(initial.agc_speed)),
            agc_attack_ms_bits: std::sync::atomic::AtomicU32::new(
                initial
                    .agc_attack_ms
                    .map(f32::to_bits)
                    .unwrap_or(Self::NONE_F32_BITS),
            ),
            agc_release_ms_bits: std::sync::atomic::AtomicU32::new(
                initial
                    .agc_release_ms
                    .map(f32::to_bits)
                    .unwrap_or(Self::NONE_F32_BITS),
    ...
    pub fn load(&self) -> AudioParams {
        let l = self.l.load(Ordering::Relaxed);
        let r = self.r.load(Ordering::Relaxed);
        let m = f64::from_bits(self.m_bits.load(Ordering::Relaxed));
        let mute = self.mute.load(Ordering::Relaxed) != 0;
        let squelch_enabled = self.squelch_enabled.load(Ordering::Relaxed) != 0;
        let demodulation = Self::u8_to_demod(self.demodulation.load(Ordering::Relaxed));
        let agc_speed = Self::u8_to_agc(self.agc_speed.load(Ordering::Relaxed));
        let agc_attack_ms = Self::bits_to_opt_f32(self.agc_attack_ms_bits.load(Ordering::Relaxed));
        let agc_release_ms =
            Self::bits_to_opt_f32(self.agc_release_ms_bits.load(Ordering::Relaxed));
    ...
    pub fn store_defaults(&self, defaults: AudioParams) {
        self.l.store(defaults.l, Ordering::Relaxed);
        self.r.store(defaults.r, Ordering::Relaxed);
        self.m_bits.store(defaults.m.to_bits(), Ordering::Relaxed);
        self.mute.store(defaults.mute as u8, Ordering::Relaxed);
        self.squelch_enabled
            .store(defaults.squelch_enabled as u8, Ordering::Relaxed);
        self.demodulation
            .store(Self::demod_to_u8(defaults.demodulation), Ordering::Relaxed);
        self.agc_speed
            .store(Self::agc_to_u8(defaults.agc_speed), Ordering::Relaxed);
        self.agc_attack_ms_bits.store(
            defaults
                .agc_attack_ms
                .map(f32::to_bits)
                .unwrap_or(Self::NONE_F32_BITS),
        ...
        self.agc_release_ms_bits.store(
            defaults
                .agc_release_ms
                .map(f32::to_bits)
                .unwrap_or(Self::NONE_F32_BITS),
    ...
    pub fn store_window(&self, l: i32, m: f64, r: i32) {
        self.l.store(l, Ordering::Relaxed);
        self.r.store(r, Ordering::Relaxed);
        self.m_bits.store(m.to_bits(), Ordering::Relaxed);
    }

    pub fn store_demodulation(&self, mode: novasdr_core::dsp::demod::DemodulationMode) {
        self.demodulation
            .store(Self::demod_to_u8(mode), Ordering::Relaxed);
    }

    pub fn store_mute(&self, mute: bool) {
        self.mute.store(mute as u8, Ordering::Relaxed);
    }

    pub fn store_squelch_enabled(&self, enabled: bool) {
        self.squelch_enabled.store(enabled as u8, Ordering::Relaxed);
    }

    pub fn store_agc(&self, speed: AgcSpeed, attack: Option<f32>, release: Option<f32>) {
        self.agc_speed
            .store(Self::agc_to_u8(speed), Ordering::Relaxed);
        self.agc_attack_ms_bits.store(
            attack.map(f32::to_bits).unwrap_or(Self::NONE_F32_BITS),
            Ordering::Relaxed,
        );
        self.agc_release_ms_bits.store(
            release.map(f32::to_bits).unwrap_or(Self::NONE_F32_BITS),
            Ordering::Relaxed,
        );
    }

    fn bits_to_opt_f32(bits: u32) -> Option<f32> {
        if bits == Self::NONE_F32_BITS {
            None
        } else {
    ...
    fn demod_to_u8(mode: novasdr_core::dsp::demod::DemodulationMode) -> u8 {
        match mode {
            novasdr_core::dsp::demod::DemodulationMode::Usb => 0,
            novasdr_core::dsp::demod::DemodulationMode::Lsb => 1,
            novasdr_core::dsp::demod::DemodulationMode::Am => 2,
            novasdr_core::dsp::demod::DemodulationMode::Sam => 3,
            novasdr_core::dsp::demod::DemodulationMode::Fm => 4,
        }
    }

    fn u8_to_demod(v: u8) -> novasdr_core::dsp::demod::DemodulationMode {
        match v {
            1 => novasdr_core::dsp::demod::DemodulationMode::Lsb,
            2 => novasdr_core::dsp::demod::DemodulationMode::Am,
            3 => novasdr_core::dsp::demod::DemodulationMode::Sam,
            4 => novasdr_core::dsp::demod::DemodulationMode::Fm,
            _ => novasdr_core::dsp::demod::DemodulationMode::Usb,
        }
    }

    fn agc_to_u8(speed: AgcSpeed) -> u8 {
        match speed {
            AgcSpeed::Default => 0,
            AgcSpeed::Off => 1,
            AgcSpeed::Fast => 2,
